...
class Text_line
{
    ...
    public:
        ...
        void Ex_string(const string &goal, const string &begoal); //替换子串 
...
{
	if (cur_line == head -> next) return -1;
	else 
	{
...
void Text_line::Ex_string(const string &goal, const string &begoal)
{
	Line_node *ptr;
	for (ptr = head->next; ptr != NULL; ptr = ptr->next)
	{
		if (ptr->line.find(begoal) != string::npos)
		{
			ptr->line.replace(0, begoal.length(), goal);
		}
	}
}
